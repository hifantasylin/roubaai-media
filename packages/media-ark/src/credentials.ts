/**
 * Key resolution for both Ark providers, in one place so the image and video
 * backends can never disagree about where a key comes from.
 *
 * The order is deliberate and identical to every other backend in this family:
 * the value a person just typed into the form (it is the most recent statement
 * of intent, and it is what lets a key be validated before it is stored), then
 * the stored Settings row, then the credential store — and through it the
 * `apiKeyEnv` environment variable.
 *
 * The `undefined` return is the interesting part: it is what lets a caller tell
 * "this deployment has no key configured" (a neutral state to show, not a
 * failure) from "a key exists and the backend refused it" (a real failure that
 * needs the vendor's reason).
 * @module @roubaai/media-ark/credentials
 */
 ...
 */
export async function resolveArkKey(
  ctx: Context,
  namespace: string,
 ...
 */
export function arkUnconfiguredReason(apiKeyEnv: string): string {
