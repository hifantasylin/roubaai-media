...
export function registerGenerateVideo(ctx: Context): () => void {
  ...
  disposers.push(ctx.tools.register(defineTool({
    name,
    description: 'Generate a short video (text-to-video / image-to-video / video_edit). Background job: returns a job id; read the completed result via job_output. The finished video is displayed automatically in the conversation as this job_output tool-result card (inline player) — do NOT paste its URL / JSON into your reply to "show" it, and never call read_image on a generated video. Persist with media_asset_save (reference = the job_output JSON or mediaRef.url) when the file must outlive the 24h URL expiry. Pass reference images via imageUrls (public https only). Caps are model-aware: 2.0 系 = 4-15s / 9 images / 3 videos / 3 audio; 2.5 = 4-30s / 30 / 10 / 10 + video_edit (size=adaptive) + outputFormat. COST per second: 2.0-fast $0.0637 (480p) / $0.137 (720p); 2.0 standard $0.0792 / $0.1704; 2.5 $0.1201 / $0.27 (含视频输入更便宜 $0.072/$0.162). Prefer 480p and 4-5s to control cost; videoUrls stacks reference billing; generateAudio adds audio output.',
    parameters: {
      prompt: { type: 'string', required: true, description: 'Video prompt (1-4000 characters).' },
      model: { type: 'string', required: true, enum: ['doubao-seedance-2.0-mini', 'doubao-seedance-2.0-fast', 'doubao-seedance-2.0', 'doubao-seedance-2.5'], description: '模型档位（必填）：用户未指定时 默认 mini 档。' },
