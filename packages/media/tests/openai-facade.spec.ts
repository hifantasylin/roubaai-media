 ...
 */
let scratch = ''
const assets = createTempAssetsRoot('roubaai-facade-')
beforeAll(async () => {
...
describe('openai facade: landing and ledger', () => {
  it('writes the generated bytes into the asset root, indexes them and records the cost', async () => {
    const { ctx, provider } = await boot()
    // A directory the provider's bytes are cached in; the asset root is the
    // spec's own, claimed above.
    ...
    expect(assetPath).toContain(join(scratch, '画布项目', '01_试验', '镜01.png'))
    expect((await readFile(assetPath)).byteLength).toBe(png.byteLength)
    expect(await readFile(join(scratch, '画布项目', 'assets-index.md'), 'utf8')).toContain('镜01.png')
    expect(await readFile(join(scratch, '画布项目', 'media-cost.jsonl'), 'utf8')).toContain('"tool":"image"')
